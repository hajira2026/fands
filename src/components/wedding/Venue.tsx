...
const MAPS_QUERY = "SK Blue Heaven";
const EMBED_SRC = `https://www.google.com/maps?q=${encodeURIComponent(MAPS_QUERY)}&output=embed`;
const DIRECTIONS = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(MAPS_QUERY)}`;

const Venue = () => (
  <section className="relative py-[120px] md:py-[160px] px-6 overflow-hidden">
    {/* Parallax mosque silhouette */}
    <ParallaxLayer depth={0.3} className="absolute inset-0 -z-10 flex items-end justify-center opacity-25">
    ...
    <div className="reveal max-w-[700px] mx-auto text-center">
      ...
      {/* Live embedded map */}
      <div className="mt-12 glass-card rounded-sm p-2 md:p-3 light-sweep"
        style={{ boxShadow: "0 0 60px hsl(46 65% 52% / 0.18), 0 30px 80px -20px hsl(173 60% 4% / 0.7)" }}>
        <div className="relative overflow-hidden rounded-sm" style={{ aspectRatio: "16/10" }}>
          <iframe
            title="SK Blue Heaven location"
            src={EMBED_SRC}
            loading="lazy"
          ...
          />
          {/* gold tint overlay (clicks pass through to iframe) */}
          <div className="pointer-events-none absolute inset-0 mix-blend-overlay"
            style={{ background: "radial-gradient(ellipse at center, transparent 50%, hsl(46 65% 52% / 0.25))" }} />
        </div>
      </div>
